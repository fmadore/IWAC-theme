...
6. **The quiet control is the default; the loud one opts in.** A bare `<button>` is outlined, flat, ink-on-transparent. The filled-primary treatment — brand fill, `--glow-sm` halo, hover lift — arrives only via `.btn--primary` or a submit control (the one button in a form that *is* the loud action). Until v2.10 this was inverted: the base selector painted every button filled-and-glowing, so sixteen component files reset `border-radius` / `box-shadow` / `transform` just to opt *out*, and any new component that overrode only background+color silently kept a rounded floating halo. A default is a statement about what the system thinks is normal, and restraint has to be the normal one.
7. **Computational honesty.** AI-generated metadata gets explicit visual treatment that signals provenance — not cosmetically blended into human-authored fields.
8. **Multilingual rigor.** FR / EN / AR transliteration get equivalent typographic treatment in every face used (test `ḥ ṣ ḍ ṭ ẓ ʿ ā ī ū` before adopting any font). Tabular figures everywhere alignment matters.
