...
**Item-page "How to cite" panel (IWAC-SEO).** The citation panel at the foot of every citable item page is rendered by the theme (`view/common/citation.phtml` + `components/citation/_citation.scss` + `asset/js/citation.js`), but the citation **data** is owned by the [IWAC-SEO](https://github.com/fmadore/IWAC-SEO) module — `view/omeka/site/item/show.phtml` calls its `iwacCitation($item)` view helper (guarded by `has('iwacCitation')`), which returns per-style formatted HTML (Chicago/APA/MLA), download URLs (`/cite/{id}/{format}` → BibTeX/RIS/CSL-JSON) and an optional Zotero-RDF link. The helper returns `null` for authority records (person/place/org/…), so the panel is automatic on citable items only. Do NOT re-implement citation formatting in the theme — the class→kind map and the formatters live in IWAC-SEO (`CitationData`/`CitationFormatter`/`CitationExport`). The panel follows "rules not boxes": a 2px ink-rule opener, Besley head, reading-serif citation, primary current-state style tabs, quiet outlined download chips.
