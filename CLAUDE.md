...
**Item-page "How to cite" panel (IWAC-SEO).** The citation panel is a **resource page block** ("How to cite") registered by the [IWAC-SEO](https://github.com/fmadore/IWAC-SEO) module and placed via Admin → Themes → Configure resource pages — NOT hard-coded in `show.phtml` (which only renders `resourcePageBlocks()`). The block delegates to the theme's UI (`view/common/citation.phtml` + `components/citation/_citation.scss` + `asset/js/citation.js`) via the module's `iwacCitation($item)` view helper, which returns per-style formatted HTML (Chicago/APA/MLA), download URLs (`/cite/{id}/{format}` → BibTeX/RIS/CSL-JSON) and an optional Zotero-RDF link. The helper returns `null` for authority records (person/place/org/…), so the block is empty there; it localises Chicago/APA/MLA connectives + dates to the site language. Do NOT re-implement citation formatting in the theme — the class→kind map and the formatters live in IWAC-SEO (`CitationData`/`CitationFormatter`/`CitationExport`). The panel follows "rules not boxes": a 2px ink-rule opener, Besley head, reading-serif citation, primary current-state style tabs, quiet outlined download chips.
